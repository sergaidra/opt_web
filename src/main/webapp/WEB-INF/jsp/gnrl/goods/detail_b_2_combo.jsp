...
<script type="text/javascript">
	$(document).ready(function(){
		...
		// combo 선택
		$(".n2_lst").click(function(){
			var objDiv = $(this).parents('div');
			var idx = objDiv.attr('id').substring(objDiv.attr('id').lastIndexOf('_')+1);

			console.log($(this).attr('id'));
			
			if($(this).attr('id').indexOf('TIME_') > -1) {
				
				console.log($(this).attr('id').replace('TIME_', ''));
				
				$("input:hidden[id=hidTime]").val($(this).attr('id'));
				$('#combo_span_'+idx).text('시간　'+$(this).text());	
			}
			
			
			$('#drop_down_'+idx).hide();
			$('#combo_list_'+idx).removeClass('btn_arrow_up').addClass('btn_arrow_down');
		});
	...
	function fnChangeComboCss(idx) {
		var objSpan = $('#combo_list_'+idx);
		var objDiv  = $('#drop_down_'+idx);
		if(objSpan.attr('class') == 'btn_arrow_down') {
			$(objDiv).show();
			$(objSpan).removeClass('btn_arrow_down').addClass('btn_arrow_up');
		} else {
			$(objDiv).hide();
			$(objSpan).removeClass('btn_arrow_up').addClass('btn_arrow_down');
		}
	}
			...
			<div class="option_box">
				...
				<div class="area_star">
					...
					<ul id="num_area">
						...
						<li class="num02">
							<span id="combo_span_1">시간</span>
							<span class="btn_arrow_down" id="combo_list_1" onclick="javascript:fnChangeComboCss('1');"></span>
							<!--드랍다운-->
							<div id="drop_down_1">
							<c:forEach var="list" items="${timeList}" varStatus="status">
								<p class="n2_lst" id="TIME_${list.TOUR_TIME}">${fn:substring(list.BEGIN_TIME,0,2)} : ${fn:substring(list.BEGIN_TIME,2,4)} ~ ${fn:substring(list.END_TIME,0,2)} : ${fn:substring(list.END_TIME,2,4)}</p>
							</c:forEach>
							</div>
						...
						<li class="num01">날짜
							...
							<input type="text" name="txtDateRange" id="txtDateRange">
							<input type="text" name="txtDateCount" id="txtDateCount" readonly onfocus="this.blur()">
							<script>
								$(function() {
