...
<div align="center">
	상위분류 <div style="height:30px;" align="center">
		<c:if test="${fn:length(upperTourClList) == 0}">
			전체
		</c:if>
	...
	상세분류 <div style="height:30px;" align="center">
		<c:if test="${fn:length(tourClList) == 0}">
			전체
		</c:if>
		<c:forEach var="result" items="${tourClList}" varStatus="status">
			<c:if test="${status.count > 1}"> - </c:if>
			<a href="javascript:fnSearchCl('${result.CL_CODE}');">#${result.CL_NM}</a>
		</c:forEach>
	</div>
