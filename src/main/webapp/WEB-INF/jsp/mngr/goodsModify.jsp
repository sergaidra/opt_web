...
<script type="text/javascript">
	...
	function f_delFile(file_code, file_sn) {
		$.ajax({
			url: "<c:url value='../delFileDetail/'/>?FILE_CODE="+file_code+"&FILE_SN="+file_sn,
			type:'post',
			datatype: 'json',
			async: true,
...
◆ 여행상품등록
<br><br>
<form id="form1" method="post" action="<c:url value='../modGoods/'/>" enctype="multipart/form-data">
<input type="hidden" name="GOODS_CODE" id="GOODS_CODE" value="${goodsInfo.GOODS_CODE}">
<input type="hidden" name="FILE_CODE" id="FILE_CODE" value="${goodsInfo.FILE_CODE}">
	...
	<tr>
		...
		<td width="75%">
			<select name="CL_CODE" id="CL_CODE">
				...
				<option value="${tourCl.CL_CODE}" <c:if test="${tourCl.CL_CODE == goodsInfo.CL_CODE}">selected</c:if>>${tourCl.CL_NM}</option></c:forEach>
			</select>
		</td>
	</tr>
	<tr>
		<td width="25%">상품명</td>
		<td width="75%"><input type="text" id="GOODS_NM" name="GOODS_NM" size="65" maxlength="200" title="상품이름" value="${goodsInfo.GOODS_NM}"></td>
	</tr>
	<tr>
		<td width="25%">상품설명</td>
		<td width="75%"><textarea id="GOODS_INTRCN" name="GOODS_INTRCN" cols="50" rows="10" title="상품설명">${goodsInfo.GOODS_INTRCN}</textarea></td>
	</tr>
	<tr>
		<td width="25%">구매가능 날짜</td>
		<td width="75%">
		<div id="divDateGroup">
		<c:forEach var="schdul" items="${schdulList}" varStatus="status">
			<div class="divDate">
				<input type="text" class="DatePickerBegin" name="BEGIN_DE" id="BEGIN_DE_<c:out value='${status.index}'/>" size="10" value="${fn:substring(schdul.BEGIN_DE,0,4)}-${fn:substring(schdul.BEGIN_DE,4,6)}-${fn:substring(schdul.BEGIN_DE,6,8)}"> ~ <input type="text" class="DatePickerEnd" name="END_DE" id="END_DE__<c:out value='${status.index}'/>" size="10" value="${fn:substring(schdul.END_DE,0,4)}-${fn:substring(schdul.END_DE,4,6)}-${fn:substring(schdul.END_DE,6,8)}">
				<c:if test="${status.index == 0}"><input type="button" value="추가" id="btnAddDate"></c:if>
				<c:if test="${status.index > 0}"><input type="button" value="삭제" id="btnDelDate"></c:if>
			</div>
		</c:forEach>
		</div>
		</td>
		...
		<c:forEach var="time" items="${timeList}" varStatus="status">
			<div class="divTime">
				<select name="BEGIN_HH" id="BEGIN_HH">
					...
					<option value="<fmt:formatNumber value="${i}" pattern="00"/>" <c:if test="${time.BEGIN_HH == i}">selected</c:if>><fmt:formatNumber value="${i}" pattern="00"/>시</option>
				</c:forEach></select> :
				<select name="BEGIN_MI" id="BEGIN_MI">
					<option value="">선택</option><c:forEach var="i" begin="0" end="50" step="10">
					<option value="<fmt:formatNumber value="${i}" pattern="00"/>" <c:if test="${time.BEGIN_MI == i}">selected</c:if>><fmt:formatNumber value="${i}" pattern="00"/>분</option>
				</c:forEach></select> ~
				<select name="END_HH" id="END_HH">
					<option value="">선택</option><c:forEach var="i" begin="0" end="23" step="1">
					<option value="<fmt:formatNumber value="${i}" pattern="00"/>" <c:if test="${time.END_HH == i}">selected</c:if>><fmt:formatNumber value="${i}" pattern="00"/>시</option>
				</c:forEach></select> :
				<select name="END_MI" id="END_MI">
					<option value="">선택</option><c:forEach var="i" begin="0" end="50" step="10">
					<option value="<fmt:formatNumber value="${i}" pattern="00"/>" <c:if test="${time.END_MI == i}">selected</c:if>><fmt:formatNumber value="${i}" pattern="00"/>분</option>
				</c:forEach></select>
				<c:if test="${status.index == 0}"><input type="button" value="추가" id="btnAddTime"></c:if>
				<c:if test="${status.index > 0}"><input type="button" value="삭제" id="btnDelTime"></c:if>
			</div>
		</c:forEach>
		</div>
		</td>
	...
	<tr>
		...
		<td width="75%">
			<select name="WAIT_HH" id="WAIT_HH">
				...
				<option value="<fmt:formatNumber value="${i}" pattern="00"/>" <c:if test="${fn:substring(goodsInfo.WAIT_TIME,0,2) == i}">selected</c:if>><fmt:formatNumber value="${i}" pattern="00"/></option>
			</c:forEach></select> 시간  &nbsp;
			<select name="WAIT_MI" id="WAIT_MI">
				<c:forEach var="i" begin="0" end="55" step="5">
				<option value="<fmt:formatNumber value="${i}" pattern="00"/>" <c:if test="${fn:substring(goodsInfo.WAIT_TIME,2,4) == i}">selected</c:if>><fmt:formatNumber value="${i}" pattern="00"/></option>
			</c:forEach></select> 분
		</td>
	</tr>
	<tr>
		...
		<td width="75%">
			<select name="MVMN_HH" id="MVMN_HH">
				<c:forEach var="i" begin="0" end="5" step="1">
				<option value="<fmt:formatNumber value="${i}" pattern="00"/>" <c:if test="${fn:substring(goodsInfo.MVMN_TIME,0,2) == i}">selected</c:if>><fmt:formatNumber value="${i}" pattern="00"/></option>
			</c:forEach></select> 시간  &nbsp;
			<select name="MVMN_MI" id="MVMN_MI">
				<c:forEach var="i" begin="0" end="55" step="5">
				<option value="<fmt:formatNumber value="${i}" pattern="00"/>" <c:if test="${fn:substring(goodsInfo.MVMN_TIME,2,4) == i}">selected</c:if>><fmt:formatNumber value="${i}" pattern="00"/></option>
			</c:forEach></select> 분
		</td>
	</tr>
<!-- 	<tr>
	...
	<tr>
		...
		<c:forEach var="mngr" items="${mngrList}" varStatus="status">
			<input type="hidden" name="NMPR_CND" id="NMPR_CND_${status.count}" value="${mngr.NMPR_CND}">${mngr.NMPR_CND} : <input type="text" name="SETUP_AMOUNT" id="SETUP_AMOUNT_${status.count}" style="text-align:right;" onkeyup="f_inputNumberFormat(this)" value="${mngr.SETUP_AMOUNT}"><br>
		</c:forEach>
		</td>
	</tr>
	<tr>
		...
		<td width="75%">
			<div id="divFile_${fileList[0].FILE_SN}">
			${fileList[0].FILE_NM} <a href="javascript:f_delFile('${fileList[0].FILE_CODE}', '${fileList[0].FILE_SN}');">삭제</a><br>
			</div>
			<input type="file" name="FILE_NM" id="FILE_NM_0" size="50">
		</td>
	</tr>
	<tr>
		...
		<td width="75%">
			<c:forEach var="file" items="${fileList}" varStatus="status">
				<c:if test="${status.index > 0 && file.FILE_CL == 'M'}">
				<div id="divFile_${file.FILE_SN}">
				${file.FILE_NM} <a href="javascript:f_delFile('${file.FILE_CODE}', '${file.FILE_SN}');">삭제</a><br>
				</div>
				</c:if>
			</c:forEach>
			<input type="file" name="FILE_NM" id="FILE_NM_1" size="50">
		</td>
	</tr>
	<tr>
		...
		<div id="divFileGroup">
			<div class="divFile">
			<c:forEach var="file" items="${fileList}" varStatus="status">
				<c:if test="${status.index > 0 && file.FILE_CL == 'I'}">
				<div id="divFile_${file.FILE_SN}">
				${file.FILE_NM} <a href="javascript:f_delFile('${file.FILE_CODE}', '${file.FILE_SN}');">삭제</a><br>
				</div>
				</c:if>
	...
	<tr>
		<td>위치</td>
		<td>
			위도 : <input type="text" name="ACT_LA" id="ACT_LA" value="${goodsInfo.ACT_LA}">&nbsp;&nbsp;&nbsp;
			경도 : <input type="text" name="ACT_LO" id="ACT_LO" value="${goodsInfo.ACT_LO}">
		</td>
	</tr>
	<tr>
		<td width="25%">바우처티켓유형</td>
		<td width="75%"><input type="text" id="VOCHR_TICKET_TY" name="VOCHR_TICKET_TY" size="65" maxlength="1" title="바우처 티켓 유형" value="${goodsInfo.VOCHR_TICKET_TY}"></td>
	</tr>
	<tr>
		<td width="25%">바우처발권소요시간</td>
		<td width="75%"><input type="text" id="VOCHR_NTSS_REQRE_TIME" name="VOCHR_NTSS_REQRE_TIME" size="65" maxlength="30" title="바우처 발권 소요 시간"  value="${goodsInfo.VOCHR_NTSS_REQRE_TIME}"></td>
	</tr>
	<tr>
		<td width="25%">바우처사용방법</td>
		<td width="75%"><input type="text" id="VOCHR_USE_MTH" value="${goodsInfo.VOCHR_USE_MTH}"
			name="VOCHR_USE_MTH" size="65" maxlength="1000" title="바우처 사용 방법"></td>
	</tr>
	<tr>
		<td width="25%">안내이용시간</td>
		<td width="75%"><input type="text" id="GUIDANCE_USE_TIME" value="${goodsInfo.GUIDANCE_USE_TIME}"
			name="GUIDANCE_USE_TIME" size="65" maxlength="100" title="안내 이용 시간"></td>
	</tr>
	<tr>
		<td width="25%">안내소요시간</td>
		<td width="75%"><input type="text" id="GUIDANCE_REQRE_TIME" value="${goodsInfo.GUIDANCE_REQRE_TIME}"
			name="GUIDANCE_REQRE_TIME" size="65" maxlength="100"
			title="안내 소요 시간"></td>
	</tr>
	<tr>
		<td width="25%">안내연령구분</td>
		<td width="75%"><input type="text" id="GUIDANCE_AGE_DIV" value="${goodsInfo.GUIDANCE_AGE_DIV}"
			name="GUIDANCE_AGE_DIV" size="65" maxlength="200" title="안내 연령 구분"></td>
	</tr>
	<tr>
		<td width="25%">안내여행일정</td>
		<td width="75%"><input type="text" id="GUIDANCE_TOUR_SCHDUL" value="${goodsInfo.GUIDANCE_TOUR_SCHDUL}"
			name="GUIDANCE_TOUR_SCHDUL" size="65" maxlength="1000"
			title="안내 여행 일정"></td>
	</tr>
	<tr>
		<td width="25%">안내공연장위치</td>
		<td width="75%"><input type="text" id="GUIDANCE_PRFPLC_LC" value="${goodsInfo.GUIDANCE_PRFPLC_LC}"
			name="GUIDANCE_PRFPLC_LC" size="65" maxlength="1000"
			title="안내 공연장 위치"></td>
	</tr>
	<tr>
		<td width="25%">안내교육과정</td>
		<td width="75%"><input type="text" id="GUIDANCE_EDC_CRSE" value="${goodsInfo.GUIDANCE_EDC_CRSE}"
			name="GUIDANCE_EDC_CRSE" size="65" maxlength="1000"
			title="안내 교육 과정"></td>
	</tr>
	<tr>
		<td width="25%">안내옵션사항</td>
		<td width="75%"><input type="text" id="GUIDANCE_OPTN_MATTER" value="${goodsInfo.GUIDANCE_OPTN_MATTER}"
			name="GUIDANCE_OPTN_MATTER" size="65" maxlength="1000"
			title="안내 옵션 사항"></td>
	</tr>
	<tr>
		<td width="25%">안내픽업</td>
		<td width="75%"><input type="text" id="GUIDANCE_PICKUP" value="${goodsInfo.GUIDANCE_PICKUP}"
			name="GUIDANCE_PICKUP" size="65" maxlength="200" title="안내 픽업"></td>
	</tr>
	<tr>
		<td width="25%">안내준비물</td>
		<td width="75%"><input type="text" id="GUIDANCE_PRPARETG" value="${goodsInfo.GUIDANCE_PRPARETG}"
			name="GUIDANCE_PRPARETG" size="65" maxlength="200" title="안내 준비물"></td>
	</tr>
	<tr>
		<td width="25%">안내포함사항</td>
		<td width="75%"><input type="text" id="GUIDANCE_INCLS_MATTER" value="${goodsInfo.GUIDANCE_INCLS_MATTER}"
	...
	<tr>
		...
		<td width="75%"><input type="text"
			id="GUIDANCE_NOT_INCLS_MATTER" name="GUIDANCE_NOT_INCLS_MATTER" value="${goodsInfo.GUIDANCE_NOT_INCLS_MATTER}"
			size="65" maxlength="500" title="안내 불포함 사항"></td>
	</tr>
	<tr>
		<td width="25%">추가안내</td>
		<td width="75%"><input type="text" id="ADIT_GUIDANCE" value="${goodsInfo.ADIT_GUIDANCE}"
			name="ADIT_GUIDANCE" size="65" maxlength="1000" title="추가 안내"></td>
	</tr>
	<tr>
		<td width="25%">유의사항</td>
		<td width="75%"><input type="text" id="ATENT_MATTER" value="${goodsInfo.ATENT_MATTER}"
			name="ATENT_MATTER" size="65" maxlength="1000" title="유의 사항"></td>
	</tr>
	<tr>
		<td width="25%">변경환불규정</td>
		<td width="75%"><input type="text" id="CHANGE_REFND_REGLTN" value="${goodsInfo.CHANGE_REFND_REGLTN}"
			name="CHANGE_REFND_REGLTN" size="65" maxlength="1000"
			title="변경 환불 규정"></td>
	</tr>
	<tr>
		<td width="25%">소개상품유형</td>
		<td width="75%"><input type="text" id="INTRCN_GOODS_TY" value="${goodsInfo.INTRCN_GOODS_TY}"
			name="INTRCN_GOODS_TY" size="65" maxlength="20" title="소개 상품 유형"></td>
	</tr>
	<tr>
		<td width="25%">소개이용시간</td>
		<td width="75%"><input type="text" id="INTRCN_USE_TIME" value="${goodsInfo.INTRCN_USE_TIME}"
			name="INTRCN_USE_TIME" size="65" maxlength="20" title="소개 이용 시간"></td>
	</tr>
	<tr>
		<td width="25%">소개집합시간</td>
		<td width="75%"><input type="text" id="INTRCN_MEET_TIME" value="${goodsInfo.INTRCN_MEET_TIME}"
			name="INTRCN_MEET_TIME" size="65" maxlength="20" title="소개 집합 시간"></td>
	</tr>
	<tr>
		<td width="25%">소개소요시간</td>
		<td width="75%"><input type="text" id="INTRCN_REQRE_TIME" value="${goodsInfo.INTRCN_REQRE_TIME}"
			name="INTRCN_REQRE_TIME" size="65" maxlength="20" title="소개 소요 시간"></td>
	</tr>
	<tr>
		<td width="25%">소개제공언어</td>
		<td width="75%"><input type="text" id="INTRCN_PROVD_LANG" value="${goodsInfo.INTRCN_PROVD_LANG}"
			name="INTRCN_PROVD_LANG" size="65" maxlength="20" title="소개 제공 언어"></td>
	</tr>
	<tr>
		<td width="25%">소개가능연령</td>
		<td width="75%"><input type="text" id="INTRCN_POSBL_AGE" value="${goodsInfo.INTRCN_POSBL_AGE}"
			name="INTRCN_POSBL_AGE" size="65" maxlength="20" title="소개 가능 연령"></td>
	</tr>
	<tr>
		<td width="25%">소개장소</td>
		<td width="75%"><input type="text" id="INTRCN_PLACE" value="${goodsInfo.INTRCN_PLACE}"
