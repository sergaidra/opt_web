...
var gridFile = Ext.create('Ext.grid.Panel', {
	...
	columns: [{
		...
		sortable: false,
		menuDisabled: true,
		editor: {xtype:'filefield', buttonText: '찾기...', regex: /^.*\.(BMP|GIF|JPG|JPEG|PNG|bmp|gif|jpg|jpeg|png)$/, regexText: '이지미 파일을 선택하세요.'},
		dataIndex: 'FILE_NM'
	},{
		text: '대표여부',
		width: 100,
	...
	tbar: [{
		...
		handler : function() {
			if(!Ext.getCmp('form-reg-goods-code').getValue()) {
				...
				var idx = storeFile.getCount();
				var r = Ext.create('GoodsFileInfo', {
					GOODS_CODE : Ext.getCmp('form-reg-goods-code').getValue(),
					FILE_SN : '',
					FILE_NM : '',
					REPRSNT_AT : 'N',
				...
				storeFile.insert(idx, r);
				cellEditing4.startEditByPosition({row: idx, column: 0});
			}
		}
	}, {
		text: '삭제',
