...
function fn_checkValue(objId) {
	...
	if(fn_getByteLength(Ext.getCmp(objId).getValue()) > len) {
		...
		Ext.Msg.alert('알림', '입력 글자수(일어기준 '+len+'자)를 초과하였습니다.', function(){Ext.getCmp(objId+'-eng').focus();});
		return false;
	} else {
		return true;
	}
...
Ext.define('GoodsInfo', {
	...
	fields: [ {name:'GOODS_CODE', type:'string'}
			...
			, {name:'GOODS_NM_ENG', type:'string'}  // 일어 컬럼
	...
	items: [{
		...
		items: [{
			...
			items: [{
				...
				//fieldLabel: '공연장위치(일어)<br>(1000자)',
				fieldLabel: '가격구성(일어)<br>(4000자)',
				fieldStyle: {'ime-mode':'disabled'},
				labelSeparator: ':',
				labelWidth: 100,
				labelAlign: 'right',
			...
			items: [{
				...
				//fieldLabel: '교육과정(일어)<br>(1000자)',
				fieldLabel: '긴급연락처(일어)<br>(4000자)',
				fieldStyle: {'ime-mode':'disabled'},
				labelSeparator: ':',
				labelWidth: 100,
				labelAlign: 'right',
	...
	items: [{
		...
		items: [{
			...
			items: [{
				...
				fieldLabel: '변경/환불규정<br>(4000자)(일어)',
				fieldStyle: {'ime-mode':'disabled'},
				labelSeparator: ':',
				labelWidth: 100,
				labelAlign: 'right',
